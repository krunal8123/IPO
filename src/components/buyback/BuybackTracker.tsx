...
import { Coins, TrendingUp, Radio } from 'lucide-react';
...
export const BuybackTracker: React.FC<BuybackTrackerProps> = ({ buybacks }) => {
  return (
    <div className="space-y-6">
      ...
      {/* Empty state if backend not loaded */}
      {buybacks.length === 0 ? (
        <div className="glass-card rounded-2xl p-10 text-center border border-slate-200 dark:border-slate-800 max-w-md mx-auto">
          ...
          <h3 className="text-base font-black text-slate-900 dark:text-white mb-1.5">
            No Buyback Data Loaded
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Buyback data is streamed live from the backend. Ensure the server is running to see active buyback opportunities.
          </p>
        </div>
      ) : (
        /* Buybacks Grid */
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    {item.logoUrl ? (
                      <img 
                        ...
                        className="w-10 h-10 rounded-xl object-contain bg-white dark:bg-slate-800 p-1 border border-slate-200 dark:border-slate-700 shadow-xs shrink-0"
                        onError={(e) => {
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                        Symbol: <span className="font-bold text-slate-700 dark:text-slate-300">{item.symbol}</span>
                        {item.issueSizeCr > 0 && <> • Issue Size: ₹{item.issueSizeCr.toLocaleString('en-IN')} Cr</>}
                        {item.issueDate && item.closeDate && (
                          <> • {item.issueDate} to {item.closeDate}</>
                        )}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    ...
                    <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 text-center">
                      ...
                      <span className="text-sm font-bold text-slate-800 dark:text-slate-200">
                        {item.currentMarketPrice > 0 ? `₹${item.currentMarketPrice}` : 'N/A'}
                      </span>
                    </div>
