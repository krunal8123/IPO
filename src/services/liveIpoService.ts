...
export const liveIpoService = {
  ...
  async getLiveIpos(): Promise<LiveFetchResult> {
    // 1. Try static data generated by GitHub Actions scraper (100% GitHub-hosted)
    try {
      const staticRes = await fetch('./data/ipos.json', { cache: 'no-cache' });
      if (staticRes.ok) {
        ...
        if (json.success && Array.isArray(json.data) && json.data.length > 0) {
          return {
            ...
            source: json.source || 'GitHub Real-Time Market Feed'
          };
        }
      }
    } catch {
    ...
    // 2. Try live Node backend API if available
    try {
      ...
      console.info('[LiveService] Server offline, utilizing local verified market cache.');
    }
  ...
  async getKfinIssues(): Promise<KfinIssue[]> {
    // 1. Try static data on GitHub Pages
    try {
      const staticRes = await fetch('./data/kfin.json', { cache: 'no-cache' });
      if (staticRes.ok) {
        const json = await staticRes.json();
        if (json.success && Array.isArray(json.data) && json.data.length > 0) {
          return json.data;
  ...
  async getMufgIssues(): Promise<MufgIssue[]> {
    // 1. Try static data on GitHub Pages
    try {
      const staticRes = await fetch('./data/mufg.json', { cache: 'no-cache' });
      if (staticRes.ok) {
        const json = await staticRes.json();
        if (json.success && Array.isArray(json.data) && json.data.length > 0) {
          return json.data;
